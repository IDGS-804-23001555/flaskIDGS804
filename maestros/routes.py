...
def registrar():
    create_form = forms.MaestroForm(request.form)
    # Cambiamos a .validate() para que no truene tu clase MaestroForm(Form)
    if request.method == 'POST' and create_form.validate(): 
...
def eliminar():
    ...
    create_form = forms.MaestroForm(request.form, obj=maest)

    if request.method == 'POST': 
        # En eliminar a veces no validamos todo el form porque los campos son readonly
